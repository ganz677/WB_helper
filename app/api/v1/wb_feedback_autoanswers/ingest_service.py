...
from app.core.models.enums import SourceType

from app.api.v1.wb_feedback_autoanswers.repositories.item_repo import ItemRepo
from app.api.v1.wb_feedback_autoanswers.repositories.answer_repo import AnswerRepo

from app.api.v1.wb_feedback_autoanswers.wb_client import WBFeedbacksClient
...
class FeedbackUseCases:
    def __init__(self, db: AsyncSession, wb: WBFeedbacksClient, llm: LLM | None = None):
        self.db = db
        self.items_repo = ItemRepo(self.db)
        self.answers_repo = AnswerRepo(self.db)
        self.wb = wb
        self.llm = llm

    async def ingest_recent_feedbacks(self) -> int:
        data = await self.wb.list_feedbacks(
            {"isAnswered": "false", "order": "dateDesc", "take": 1000}
        )
    ...
    async def generate_short_answer(self, item_id: int, text: str, provider_name: str) -> Answer:
        assert self.llm is not None, "LLM client is required for generation"

        reply = await self.llm.short_reply(text)

        ans = Answer(item_id=item_id, text=reply, provider=provider_name)
    ...
    async def send_answer_for_feedback(self, item_id: int, text: str) -> None:
